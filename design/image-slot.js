...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    // auto-aspect: once an image is in the slot, size the box to the image's
    // natural aspect ratio so nothing is cropped. Before a drop (or when the
    // mode is off) fall back to the author's `aspect` attribute, then to
    // whatever aspect-ratio the inline style declares.
    _syncAspect() {
      if (!this.hasAttribute('auto-aspect')) return;
      const iw = this._img.naturalWidth, ih = this._img.naturalHeight;
      // height:auto lets aspect-ratio resolve the cross-axis from width —
      // without it the shadow stylesheet's default :host height wins.
      this.style.height = 'auto';
      if (this.hasAttribute('data-filled') && iw && ih) {
    ...
    constructor() {
      ...
      this._img.addEventListener('load', () => { this._syncAspect(); this._applyView(); });
